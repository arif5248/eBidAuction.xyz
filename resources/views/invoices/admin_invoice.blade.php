		...
		<div style="background: #eceff4;padding: 1.5rem;">
			<table>
				<tr>
					...
					<td style="font-size: 2.5rem;" class="text-right strong">{{ translate('ADMIN INVOICE') }}</td>
				</tr>
			</table>
			<table>
				<tr>
				...
				<tr>
					...
					<td class="text-right small"><span class="gry-color small">{{ translate('Payment type') }}:</span> <span class=" strong">{{ $order->payment_type }}</span></td>
				...
				<tr>
					...
					<td class="text-right small"><span class="gry-color small">{{ translate('Payment Status') }}:</span> <span class=" strong">{{ $order->payment_status }}</span></td>
	        ...
	        <table style="width: 40%;margin-left:auto;" class="text-right sm-padding small strong">
		        <tbody>
			        ...
			        <tr class="border-bottom">
			            ...
			            @php $total = $order->orderDetails->sum('price');

			            $seller = \App\OrderDetail::where('order_id',$order->id)->first();
			            $user = \App\User::where('id',$seller->seller_id)->first();
			            $user_commission = $user->commission;
			            $percent = $user_commission/100;
			            $commission = $total*$percent;
			            @endphp
			            
			            <td class="currency">{{$commission}}</td>
			        </tr>
			        <tr>
			            <th class="text-left strong">{{ translate('To be Paid') }}</th>
			            <td class="currency">{{ single_price($order->grand_total - $commission) }}</td>
			        </tr>
		        </tbody>
		    </table>
	    </div>
