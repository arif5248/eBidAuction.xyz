...
@endif
<div class="product">
        @if (count($nearest_shops) > 0)
            <div class="title">{{translate('Nearest Shops')}}</div>
            <ul>
                @foreach ($nearest_shops as $key => $near_shop)
                    <li>
                        <a href="{{ route('shop.visit', $near_shop->slug) }}">
                            <div class="d-flex search-product align-items-center">
                                <div class="image" style="background-image:url('{{ my_asset($near_shop->logo) }}');">
                                </div>
                                <div class="w-100 overflow--hidden ">
                                    <div class="product-name text-truncate heading-6 strong-600">
                                        {{ $near_shop->name }}
                                        ...
                                        <div class="stock-box d-inline-block">
                                            @if($near_shop->user->seller->verification_status == 1)
                                                <span class="ml-2"><i class="fa fa-check-circle" style="color:green"></i></span>
                                            @else
                                                <span class="ml-2"><i class="fa fa-times-circle" style="color:red"></i></span>
                                            @endif
