            ...
            <tbody>
                @foreach($brands as $key => $brand)
                    <tr>
                        ...
                        <td>@if($brand->status == '1')<span class="text-success">Approved</span>@else <span class="text-danger">Not Approved</span> @endif </td>
                        <td>
                            <div class="btn-group dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    @if($brand->status=='0')
                                    <li><a href="{{route('brands.approve', encrypt($brand->id))}}">{{translate('Approve')}}</a></li>
                                    @endif
                                    <li><a href="{{route('brands.edit', encrypt($brand->id))}}">{{translate('Edit')}}</a></li>
                                    <li><a onclick="confirm_modal('{{route('brands.delete', $brand->id)}}');">{{translate('Delete')}}</a></li>
                                </ul>
                            </div>
                        </td>
                    </tr>
