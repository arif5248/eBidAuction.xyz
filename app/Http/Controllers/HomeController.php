...
class HomeController extends Controller
{
    ...
    public function product(Request $request, $slug)
    {
        ...
        $max_bidding = Bid::where('product_id',$detailedProduct->id)->max('price');
        ...
        if($detailedProduct!=null && $detailedProduct->published){
            ...
            if($detailedProduct->digital == 1){
                return view('frontend.digital_product_details', compact('detailedProduct','max_bidding'));
            }
            else {
                return view('frontend.product_details', compact('detailedProduct','max_bidding'));
            }
    ...
    public function ajax_search(Request $request)
    {
        ...
        foreach($products as $product)
            {
                $new_arr[] = $product->user_id;
            }
            $nearest_shops          =       Shop::whereIn('user_id', $new_arr);

            $nearest_shops          =       $nearest_shops->select("*", DB::raw("6371 * acos(cos(radians(" . $request->latitude . "))
                                    * cos(radians(latitute)) * cos(radians(longitute) - radians(" . $request->longitude . "))
                                    + sin(radians(" .$request->latitude. ")) * sin(radians(latitute))) AS distance"));
            $nearest_shops          =       $nearest_shops->having('distance', '<', 20);
            $nearest_shops          =       $nearest_shops->orderBy('distance', 'asc');
            $nearest_shops          =       $nearest_shops->get()->take(3);
             // $nearest_shops = Shop::whereIn('user_id', $new_arr)->orderBy('latitute','asc')->get()->take(3);
             // dd($nearest_shops);
        }
        ...
        if(sizeof($keywords)>0 || sizeof($subsubcategories)>0 || sizeof($products)>0 || sizeof($shops) >0){
            return view('frontend.partials.search_content', compact('products', 'subsubcategories', 'keywords', 'shops','nearest_shops'));
        }
        return '0';
    }
    ...
    public function referFriends(){
        return view('frontend.refer_friends');
    }

    public function referBonus(){
        $bonuses = ReferBonus::Paginate();
        return view('sales.refer_bonus',compact('bonuses'));
    }

    public function editReferBonus($id){
        $edit_bonus = ReferBonus::find($id);
        return view('sales.bonus_edit',compact('edit_bonus'));
    }
    public function updateReferBonus(Request $request,$id){
        $edit_bonus = ReferBonus::find($id);
        $edit_bonus->bonus_name = $request->bonus_name;
        $edit_bonus->amount = $request->amount;
        $edit_bonus->save();
        flash(translate('Bonus Edited successfully'))->success();
        return redirect(route('refer.bonus'));
    }
}
